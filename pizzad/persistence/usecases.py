...
from .abc import RestoreableRegistry, UserRegistryBuilder, Archive, SerializedDataBuilder


def restore_user_registry_from_url(
        builder: UserRegistryBuilder, archive: Archive, url: str) -> RestoreableRegistry:
    archive.connect(url)
    data = archive.get()
...
def save_user_registry_to_url(
        builder: SerializedDataBuilder, archive: Archive, registry: RestoreableRegistry, url: str):
