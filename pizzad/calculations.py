...
def calculate_pizzas(
        pizza_radius: float, num_people: int,
        portion_size: str = VALID_PORTION_SIZES[0]):

    assert num_people >= 0, "Number of people needs to be positive."
    assert pizza_radius >= 0, "Pizza radius needs to be positive."
    assert portion_size in VALID_PORTION_SIZES, f"Portion size '\
            '{portion_size}' is not in {VALID_PORTION_SIZES}"
